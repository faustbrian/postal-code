...
describe('Country', function (): void {
    ...
    describe('fallbackPostalCode', function (): void {
        ...
        test('returns digits-only fallback format', function (): void {
            expect(Country::fallbackPostalCode())->toMatch('/^\d{5}$/');
        });
    });
    ...
    describe('countriesWithoutPostalCodes', function (): void {
        ...
        test('contains no duplicate country codes', function (): void {
            $countries = Country::countriesWithoutPostalCodes();

            expect($countries)->toHaveCount(count(array_unique($countries)));
        });
